      ...
      <Link as={`/writings/${slug}`} href="/writings/[slug]">
        <div>
          <div className="hidden gap-4 my-4 cursor-pointer md:flex-row md:flex group">
            <div>
              <div className="mb-2 text-lg font-medium leading-snug text-white underline transition duration-500 transform group-hover:opacity-70">
                {title}
              </div>
              <p className="mb-2 text-base leading-snug">{excerpt}</p>
              <p className="text-base text-grey-secondary">{udate}</p>
            </div>
          </div>
